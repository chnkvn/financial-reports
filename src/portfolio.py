...
class Portfolio:
    ...
    def __attrs_post_init__(self):
        ...
        self.dict_of_assets = ({a['isin']: Asset.from_boursorama(
            get_current_asset_data(a['url'])
                               ...
                               if Path(self.jsonl_ptf_path).is_file() else {})
    ...
    def load_operations(self) -> pd.DataFrame:
        ...
        if not db_exists:
            ...
            operations['date'] = pd.to_datetime(operations['date']).dt.strftime(DATE_FORMAT)
            ic(operations.info())
    ...
    def assets_summary(self) -> pd.DataFrame:
        ...
        if self._assets_summary is None:
            ...
            for isin in df['isin'].unique():
                ...
                order by date""").df()
                quantity, total_dividends, isin_df['cumulative_quantity'] = self.get_asset_quantity(isin_df)
                summary = {'name': self.dict_of_assets[isin].name,
                           ...
                           f'IRR {TODAY.year-1}': self.compute_xirr_pv(isin_df,isin,period=f'{TODAY.year-1}'),
            ...
            self._assets_summary['proportion (%)'] = round(
                100*self._assets_summary['valuation']/
    ...
    def compute_xirr_pv(self, df: pd.DataFrame, isin: Union[str, None] = None,
    ...
    def asset_values(self):
        ...
        if self._asset_values is None:
            ...
            self._asset_values['date'] = pd.to_datetime(self._asset_values['date']).dt.date
    ...
    def portfolio_summary(self):
        ...
        if self._portfolio_summary is None:
            ...
            ptf_summary = {
                ...
                'Total invested amount': self.assets_summary['Total invested amount'].sum(),
                'IRR ytd': self.compute_xirr_pv(cum_quantities_df, period='ytd'),
                f'IRR{TODAY.year-1}': self.compute_xirr_pv(cum_quantities_df, period=f'{TODAY.year-1}'),
                'IRR since 1st buy': self.compute_xirr_pv(cum_quantities_df),
            }
            ptf_summary['Capital gain (%)'] = (100* (ptf_summary['valuation']
